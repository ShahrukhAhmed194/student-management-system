<x-student-layout>
 <div class="pagetitle">
      <div class="row">
          <div class="col-xl-4"><h1 class="text-left">{{!empty($new_session->sessionClass)? $new_session->sessionClass->name : 'Invalid Class'}}</h1></div>
          <div class="col-xl-2"><h1 class="text-center">{{!empty($attendance)? $attendance->student->name : ''}}</h1></div>
          <div class="col-xl-6">
            <h1 class="text-right">
              @if($new_session->status != 'Completed')
              {{-- <a type="button" class="btn btn-outline-success" target="_blank"  href="https://lead.academy/meeting-join-sdk?live_id={{empty($new_session)?'':$new_meeting->id}}&username={{auth()->user()->name}}&redirecturl={{ trim(redirect()->back()->getTargetUrl() ,'https://') }}">Join Now</a> --}}
              @endif
            </h1>
          </div>
      </div>       
    ...
    <div class="coming-soon-card">
      ...
      <div class="card-inner">
        <div class="header text-center">
          <h4>{{$new_session->status == 'Completed' ? 'Uploading.....' : 'Class In Session'}}</h4>
      </div>
  ...
  <section>
    <div class="row">
      <?php foreach($classSessionRecords as $record){ ?>
        <div class="col-xl-4 col-lg-4 col-md-4 col-sm-6">
          <div class="card">
            <div class="card-body">
              <h5 class="card-title">{{$new_session->sessionClass->name}}</h5>
              <iframe src="{{ $record->recording_link }}" width="100%" height="300" frameborder="0" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen title="{{ $new_session->sessionClass->name }}"></iframe>
            </div>
          </div>
        </div>
      <?php } ?>
          ...
          <div class="col-xl-3 col-lg-3 col-md-4 col-sm-6">
              <div class="card info-card sales-card " style="background-color: #EF5C48">
                  <div class="card-body" style="background-color: #0183CC" >
                      <h6 class="card-title font-weight-bold text-light p-4">{{$new_session->status == 'Completed' ? 'Ended' : 'Will End(EST)'}}</h6>
                    <div class="d-flex align-items-center">
                      <div class="card-icon rounded-circle d-flex align-items-center justify-content-center ">
