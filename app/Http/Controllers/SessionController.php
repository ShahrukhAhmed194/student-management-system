...
use App\Models\User;
use App\Models\{StudentClassParticipant, StartClassSession, ClassSessionRecord, TempSessionRecord};
...
class SessionController extends Controller
{
        ...
        if($class->teacher_id == auth()->user()->id){
            ...
            if($count >0){
                ...
                if($class->isZoomAutomated == 1){
                    ...
                    $meeting_id = $response['data']['id']; //(!empty($response['data']['id']) ? $response['data']['id'] : '');
                ...
                // --------------------- its close for meeting create in zoom panel ----------------
                    ...
                    $new_session->meeting_id    =   $meeting_id;
    ...
    public function goToStudentSession($id)
    {
        ...
        $getStartClasseSessions = StartClassSession::where('class_session_id', $class_session_id)
                    ->where('status', 'completed')
                    ->get();
                    
        $classSessionRecords = array();
        if($getStartClasseSessions){
            $sessionUuids = [];
            foreach($getStartClasseSessions as $singleSession){
                $sessionUuids[] = $singleSession->uuid;
            }
            
            $classSessionRecords = ClassSessionRecord::whereIn('uuid', $sessionUuids)
                                    ->where('status', 'completed')
                                    ->get();
        }
        ...
        //     $new_session->save();
        // }
        return view('sessions.student', compact('id', 'new_session', 'attendance', 'projects', 'today', 'new_meeting', 'classSessionRecords'));
        }else{
            return abort(403);
        }
    }
    ...
    {
        if (!auth('web')->user()->can('codingforkids.list')) {
            abort(403, 'Sorry !! You are Unauthorized to view session list !');
        }
        $classes = Student::
    ...
    public function showSessionDetailsToParent(Request $request)
    {
        ...
        foreach($students as $single){
            $classSessionIds[] = $single->id;
        }
        $getClassSessionRecords = ClassSessionRecord::whereIn('class_session_id', $classSessionIds)->where('status', 'completed')->orderBy('id', 'desc')->get();
        
        return view('sessions.all-sessions-parent', compact('students', 'getClassSessionRecords'));
    }
    ...
    public function goToParentSessionSpecific(Request $request)
    {
        ...
        $today = $request->date;
        $class_session_record_id = $request->class_session_record_id;

        // $new_session = ClassSession::where('session_date', $today)
        //             ->where('class_id', $class_id)
        //             ->first();
        $new_session = ClassSessionRecord::where('id', $class_session_record_id)
                    ->where('status', 'completed')
                    ->first();
    ...
    {
        if(auth()->user()->user_type ==  User::USER_TYPE_SUPER_ADMIN){
    ...
    public function saveSessionAsCompleted(Request $request)
    {
      ...
      ClassSession::where('session_id', $id)
        ...
        $this->meetingDeleteFromZoom($meeting_id);

       return 1;
    }

    public function meetingDeleteFromZoom($meeting_id){
            $accessToken = $this->zoomServerToServerAccessTokenGenerate();
            $curl = curl_init();
            ...
            CURLOPT_HTTPHEADER => array(
                ...
                'Cookie: __cf_bm=SiXW27lb4FJ.EiiHwNdr09VfN7MpvlOaru3_QrB_CWo-1722487084-1.0.1.1-MTJZe8E1Mj1PAnl7JkYH8lKDfPvHKqyCUWKwzvqGTaOlBWZSD62PPK3OiWUrlqPg2ouWhDyVeZxLXVgomgJuTw; _zm_csp_script_nonce=yuP5hETWSe2VNJsLNWoSzg; _zm_join_utid=UTID_c4068c613c37483b85dbb3c14f64a0c4; _zm_mtk_guid=207431abad494e3eab660ec6f0778f53; _zm_page_auth=us06_c_rNLJtxeTQSyD7iQWlkJ4iQ; _zm_ssid=us06_c_xp1FyNZrTKWdn99bwPSfeA; _zm_visitor_guid=207431abad494e3eab660ec6f0778f53; cred=A788D4F68AFBA342A664810D51313835'
            ),
            ));
    ...
    {
        if (!auth('web')->user()->can('recording.view')) {
            abort(403, 'Sorry !! You are Unauthorized to view recording list !');
        }
        if(empty($request->email) && empty($request->date)){
            ...
            if($request->email){
                $getUserInfo = User::where('email', $request->email)->where('user_type', User::USER_TYPE_TEACHER)->first();
    ...
    public function updateLiveVideoSingle(Request $request)
    {
        ...
        if($request->is_trial == 'false'){
            ...
            $recording_link = (!empty($iframe_links[1]) ? $iframe_links[1] : '');
            ...
            $getStartClassSession = StartClassSession::where('class_session_id', $request->session_id)->first();
            if($getStartClassSession){
                StartClassSession::where('class_session_id', $request->session_id)->update([
                    'status' => 'Completed',
                ]);
                $uuid = $getStartClassSession->uuid;
                ClassSessionRecord::updateOrInsert(
                    ['uuid' => $uuid],
                    [
                        'recording_link' => $recording_link,
                        'status' => 'Completed',
    ...
    public function passed_recording_edit($id){
        ...
        $tacherEmail = $getPassedRecording->teacher->email;
        
        
        $query = ZoomRecordDA::whereDate('recording_start', $session_date)->where('topic', $tacherEmail);
        $recordings = $query->get();
    ...
    public function passed_recording_update(Request $request){
        ...
        if($getZoomRecordDAInfo){
            ...
            $getStartClassSession = StartClassSession::where('class_session_id', $request->id)->first();
            if($getStartClassSession){
                StartClassSession::where('class_session_id', $request->id)->update([
                    'status' => 'Completed',
                ]);
                $uuid = $getStartClassSession->uuid;
        ...
        $recordingDate = date('Y-m-d', strtotime($getZoomRecordDAInfo->recording_start));
        toastr()->success('Updated Successfully.');
        return redirect("passed-recording?meeting_date=$recordingDate");
    }
    ...
    public function add_new_session(){
        $getTeacherInfo = User::where('user_type', User::USER_TYPE_TEACHER)->get();
        $allResults = array(
    ...
    public function session_save(Request $request){
        ...
        $classSessionData = ClassSession::create([
        ...
        StartClassSession::create([
            'class_session_id' => $classSessionData->id,
            'uuid' => $uuid,
            'meeting_id' => null,
            'start_url' => null,
            'join_url' => null,
    ...
    public function getZoomMeetingInfoByPmid($pmid){
        $accessToken = $this->zoomServerToServerAccessTokenGenerate();
        
        $curl = curl_init();

        curl_setopt_array($curl, array(
        CURLOPT_URL => "https://api.zoom.us/v2//meetings/$pmid",
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 0,
        ...
        CURLOPT_HTTPHEADER => array(
            "Authorization: Bearer $accessToken",
            'Cookie: __cf_bm=YpLkHR2iZfouiu8xbBb5OgefLQF_YdfyqH8UvInIAKs-1719307224-1.0.1.1-lPeq1jVpG0TQH7ZaDJE57jNxru9F_RKwhUo5DMf0vbq8m2_FvP45UwizKuIKSabQO09.CjwkuQVn7fNUq5juOA; _zm_mtk_guid=207431abad494e3eab660ec6f0778f53; cred=0D34965E1FB7574719D542BAF06AB340'
        ),
        ));
    ...
    public function classSessionStart(Request $request){
        $class_session_id = $request->class_session_id;
        $teacherMeetingId = $request->teacherMeetingId;
        
        $meetingResponse = $this->getZoomMeetingInfoByPmid($teacherMeetingId);
        
        if($meetingResponse->status == 'waiting'){

            ClassSession::where('id', $class_session_id)->update([
                'class_status' => 1, // 1 for host
            ...
            // $checkClassSessionRecord = ClassSessionRecord::where('class_session_id', $class_session_id)->first();
            // if(@$checkClassSessionRecord->class_session_id != $class_session_id){
                ...
                StartClassSession::create([
                    ...
                    'start_url' => $meetingResponse->start_url,
                    'join_url' => $meetingResponse->join_url,
                    'zoom_response' => json_encode($meetingResponse),
                    'status' => $meetingResponse->status,
                ]);
    
                TempSessionRecord::create([
                    'class_session_id' => $class_session_id,
                    'uuid' => $meetingResponse->uuid,
                    'meeting_id' => $meetingResponse->id,
                ]);
        ...
        return $meetingResponse->start_url;
    }

    public function classSessionMigrate(){
        ini_set('max_execution_time', 1000);
        DB::statement('SET FOREIGN_KEY_CHECKS=0;');
        DB::table('class_session_records')->truncate();
        DB::table('start_class_sessions')->truncate();

        $classSessionData = ClassSession::get();
        
                foreach($classSessionData as $single){
                    $uuid = $this->getUniqueId(32);
                                        
                    StartClassSession::create([
                        'class_session_id' => $single->id,
                        'uuid' => (!empty($single->uuid) ? $single->uuid : $uuid),
                        'meeting_id' => $single->meeting_id,
                        'start_url' => $single->start_url,
                        'join_url' => $single->join_url,
                        'zoom_response' => $single->zoom_response,
                    ...
                    ClassSessionRecord::create([
                        ...
                        'recording_uploaded' => '', 
                        'recording_deleted' => '', 
                        'recording_link' => (!empty($single->recording_link) ? $single->recording_link : null), 
                        'status' => (!empty($single->status) ? $single->status : null), 
                        'created_at' => (!empty($single->created_at) ? $single->created_at : null), 
                        'updated_at' => (!empty($single->updated_at) ? $single->updated_at : null), 
                    ]);
                }
                // ================ close ==============
                echo 'All class session record migration done';
    }

    public function testLiveClass(){
        // dd(auth()->user()->email);
        $data = array(
            'topic' => 'Live session Two', //auth()->user()->zoom_topic,
            'duration' => 120,
            'agenda' => "Coding Class",
            'start_time' => "20:00",
            'teacherEmail' => 'leadacademyinstructor@gmail.com', //auth()->user()->email,
            'host_video' => 1,
            'participant_video' => 1,
        );
        $response = $this->create($data);
